...
* 
=======
* Circular buffers by definition is known as a fixed size buffers that function as if the computer memory is contiguous, but also in a circular notion. When a memory generated and comsumed with other data, the head or tail pointers are adjusted instead of reshuffled the data itself. "When data is added, the head pointer advances... data is consumed, the tail pointer advance" (embeddedartistry).

*Ciruclar Container Type:
	- In C porgramming, the container structure is to manage the state of the buffer. For the encapsulation to preserve the data, the container have to define in the file library.
	- The contain is designed to underlying the data buffer, the maximum sze, the curent head/tail position in the queue, and an indicator flag if the buffer full or not. 
	
* A standard circular buffer use FIFO, whereas non-circular buffer use LIFO. The best wya to expanding queues in circular buffer is to use linked list method. 
>>>>>>> 7291ff609f24c99cbc881cac5a1f86f4d4f5cfda
