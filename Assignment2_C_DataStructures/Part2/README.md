...
* Stack Data Structure is a linear data structure that follow any particular order of operation. Unlike tradictional Circular queue use FIFO, Stack data structure uses either LIFO or FIFO. The main basic operation that stack use are: Push, Pop, Peep, and isEmpty. Time complexity wise, stack do not run any loop within it main baisc operatons. The two way that we can implement stack is to use either an array method or a linked list method.

* To an extend, stack data structure is the foundation of recuirsion. From time to time, a stack is consister as memory blocker cells with a fixed  bottom location, and a pointer that hold the top cell position. The terminology are being used as an irrespective of memory growth or addressed the higher memory.   
